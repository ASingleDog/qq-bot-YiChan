...
async def private_msg_handler(msg: str, sender: dict):
    if re.findall('备忘录', msg):
        return {'reply': '备忘录功能已迁移至伊酱姐妹bot，去找她看看吧~'}
    asyncio.create_task(chatgpt(msg, sid=sender['user_id']))


async def group_msg_handler(msg: str, gid: int, sender: dict):
    # super Function with #
    if re.findall(r'#', msg) and not re.findall(r'手机(QQ|qq)', msg) and not re.findall(r'&#', msg):
        msg = re.sub(r'#', '', msg)
        msg = re.sub(r'\[CQ:.*]', '', msg)
        # AI绘画
        ...
        # gpt
            ...
            asyncio.create_task(chatgpt(msg, gid=gid))
            return {}
    ...
    if re.findall(str(config['self-addressing']), msg) \
...
async def uncaughtmsg_handler(msg: str, gid: int, sender: dict) -> str:
    ...
    # 不是@的话不明白意思就不要回答
    if re.findall(r'(听[清懂])|(不支持)|(知识盲区)|(其他)|(再说)|(慢点说)|(听到了?一点)|(问题)|(学习)|(问住)', ret_msg):
        return ''
...
async def chat_handler(msg: str, gid: int, sender: dict) -> str:
    ...
    # 判断是否被手动退出聊天
    if re.findall(r'(不是[和跟叫说]你)|(没有?[和跟叫说]你)|(滚)|(爬)|(一边(呆着)?去)|(别掺和)|(退出聊天)|((不是|没)谈论你)', msg):
        exit_words = [f"对不起，{config['self-addressing']}这就走，{config['self-addressing']}下次会做个听话的乖孩子的……",
                      f"对不起，{config['self-addressing']}这就离开，请不要讨厌{config['self-addressing']}……"]
    ...
    res_text = await xiaoai(msg)
    if random.randint(0, 100) < 70 and not re.findall(r'(听[清懂])|(不支持)|(知识盲区)|(其他)|(再说)|(慢点说)|(听到了?一点)|(问题)|(学习)|(问住)', res_text):
        return res_text
    else:
        return f'[CQ:image,file={await emoji(msg)}]'
